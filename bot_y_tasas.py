...
# ---------------------------------------------------------------------
# 3) Guardar tasas en Supabase (upsert por fecha + tipo_tasa)
# ---------------------------------------------------------------------
def guardar_tasas(tasa_bcv: float | None, tasa_binance: float | None) -> None:
    ...
    if tasa_bcv:
        filas.append({"fecha": hoy, "tipo_tasa": "BCV", "valor_usd": tasa_bcv})
    if tasa_binance:
        filas.append({"fecha": hoy, "tipo_tasa": "Binance", "valor_usd": tasa_binance})
    ...
    resultado = (
        ...
        .upsert(filas, on_conflict="fecha,tipo_tasa")
        .execute()
    )
    print(f"Tasas guardadas: {filas}")
    return resultado
